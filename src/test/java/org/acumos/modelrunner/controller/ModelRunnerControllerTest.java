...
public class ModelRunnerControllerTest extends ModelRunnerTestApp {
	...
	public void predictTest() throws Exception {
		...
		try {
			...
			MockMultipartFile sampleConfigJson = new MockMultipartFile("modelConfig", "sampleConfigJson.properties", null,
					configbytesJson);
			MockMultipartHttpServletRequestBuilder builderConfigJson = MockMvcRequestBuilders.fileUpload("/model/configuration");
			...
			MockHttpServletRequestBuilder builderTransformJson = MockMvcRequestBuilders.fileUpload("/transformJSON").file(jsondata)
					.file(modelFile).file(protoJson0).param("operation", "testJSON");

			byte[] resultsTransformJson0 = this.mockMvc.perform(builderTransformJson).andExpect(status().isOk()).andReturn()
					.getResponse().getContentAsByteArray();
			...
			builderTransformJson = MockMvcRequestBuilders.fileUpload("/transformJSONDefault").file(jsondata).param("operation", "testJSON");
			resultsTransformJson0 = this.mockMvc.perform(builderTransformJson).andExpect(status().isOk()).andReturn().getResponse()
					.getContentAsByteArray();
			...
			MockHttpServletRequestBuilder builderGBJ = MockMvcRequestBuilders.fileUpload("/getBinaryJSON").file(inpJsonfile)
					.file(protoTestJson);
			...
			byte[] resultsBinDefaultJson = this.mockMvc.perform(MockMvcRequestBuilders.fileUpload("/getBinaryJSONDefault")
					.file(inpJsonfile).param("operation", "predict")).andExpect(status().isOk()).andReturn()
					.getResponse().getContentAsByteArray();
			...
			byte[] resultsPredictJSON = this.mockMvc
					.perform(post("/predict").contentType(MediaType.TEXT_PLAIN)
							.content(resultsBinDefaultJson))
					.andExpect(status().isOk()).andReturn().getResponse().getContentAsByteArray();
